...
# Initialize rate limiter with key_func
limiter = Limiter(app, key_func=get_remote_address)
limiter.init_app(app)  # Initialize it later

# Environment variables
AES_KEY = os.getenv("NOT_MY_KEY").encode("utf-8")
...
def store_private_key():
    try:
        ...
        logging.error(f"Error storing private key: {str(e)}")
        return jsonify({"error": str(e)}), 500
...
def retrieve_private_key():
    try:
        ...
        logging.error(f"Error retrieving private key: {str(e)}")
        return jsonify({"error": str(e)}), 500
...
# Handle unsupported HTTP methods (PUT, PATCH, DELETE, HEAD)
@app.route('/<path:path>', methods=['PUT', 'PATCH', 'DELETE', 'HEAD'])
def handle_method_not_allowed(path):
